...
// Create APIM named values for encryption keys using secure random values generated per deployment
// These are used for AES encryption in the OAuth callback policy
resource encryptionKeyNamedValue 'Microsoft.ApiManagement/service/namedValues@2021-08-01' = {
  parent: apimService
  name: 'EncryptionKey'
  properties: {
    displayName: 'EncryptionKey'
    value: base64(uniqueString(resourceGroup().id, apimServiceName, location, 'encryption-key'))
    secret: true
  }
}
...
resource encryptionIVNamedValue 'Microsoft.ApiManagement/service/namedValues@2021-08-01' = {
  ...
  properties: {
    ...
    value: base64(uniqueString(resourceGroup().id, apimServiceName, location, 'encryption-iv'))
    secret: true
  }
}
...
resource oauthTokenPolicy 'Microsoft.ApiManagement/service/apis/operations/policies@2021-08-01' = {
  ...
  properties: {
    format: 'rawxml'
    value: loadTextContent('token.policy.simple.xml')
  }
}
...
resource oauthCallbackPolicy 'Microsoft.ApiManagement/service/apis/operations/policies@2021-08-01' = {
  ...
  properties: {
    format: 'rawxml'
    value: loadTextContent('oauth-callback.policy.simple.xml')
  }
  dependsOn: [
