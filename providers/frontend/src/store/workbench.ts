...
export interface WorkbenchSubmission {
  ...
  remoteSubmission?: Record<string, unknown> | null;
  remoteDispatch?: Record<string, unknown> | null;
  remoteMeasurements?: WorkbenchSubmissionMeasurement[];
}
...
export async function getWorkbenchSubmission(
  ...
  const q = refresh ? "" : "?refresh=false";
  const data = await api<{ submission: WorkbenchSubmission }>(
    `/submissions/${encodeURIComponent(submissionId)}${q}`,
  );
  return data.submission;
}

export async function refreshWorkbenchSubmission(
  submissionId: string,
): Promise<WorkbenchSubmission> {
  const data = await api<{ submission: WorkbenchSubmission }>(
    `/submissions/${encodeURIComponent(submissionId)}/refresh`,
    { method: "POST" },
  );
  return data.submission;
}
