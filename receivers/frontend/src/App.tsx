...
function branchOf(pathname: string): "catalog" | "cohorts" {
  if (pathname === "/cohorts" || pathname.startsWith("/cohorts/")) return "cohorts";
  return "catalog";
}
...
function App() {
  ...
  const navTab = (
    ...
    <NavLink
      ...
      to={to}
      className={() => {
        const active = branchOf(window.location.pathname) === branch;
      ...
      aria-current={branchOf(window.location.pathname) === branch ? "page" : undefined}
    >
      <span>{label}</span>
