...
// Top-level nav tabs are bound to URL segments. The detail routes
// (e.g. /measures/:id) reuse the same page component as the list route
// (/catalog) so the nav-tab highlight should stay on "Catalog" for all of
// them. We treat any path that's not /cohorts(/...) as the catalog branch.
function tabIsActive(pathname: string, branch: "patients" | "catalog" | "cohorts"): boolean {
  ...
  if (branch === "patients") return onPatients;
  if (branch === "cohorts") return onCohorts;
  return !onPatients && !onCohorts;
}
...
function App() {
  ...
  const navTab = (
    to: string,
    branch: "patients" | "catalog" | "cohorts",
    label: string,
    sub: string,
  ) => (
    <NavLink
      ...
      to={to}
      className={({ isActive }) => {
        // react-router's isActive matches the exact path, but we also want
        // the Catalog tab lit on /measures/:id, /tags/:id, etc.
        ...
        <div className="flex flex-col lg:flex-row lg:space-x-8 mt-8">
          <section className="w-full max-w-none lg:max-w-7xl xl:max-w-none mx-auto flex-grow mt-8 lg:mt-0">
            <nav
              ...
              aria-label="Primary"
            >
              {navTab("/patients", "patients", "Patients", "SOAP · Encounters · Measures")}
