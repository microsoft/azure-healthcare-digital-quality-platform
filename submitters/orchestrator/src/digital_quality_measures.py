...
def evaluate_single_measure_cql(
    ...
    # Preserve the catalog id (e.g. "ePC02v1") on the way out so callers
    # that key per-measure results by the id they POSTed (which is the
    # cohort/catalog id, not the executor's canonical id "ePC-02") still
    # find their row. Only fall back to the executor's canonical id when
    # the catalog id is missing.
    return MeasureResult(
        measure_id=measure_def.measure_id or result.measure_id,
