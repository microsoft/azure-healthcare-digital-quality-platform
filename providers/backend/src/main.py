...
def _evaluate_bp_measurement_via_orchestrator(
    ...
    *,
    ...
    # Providers stack is orchestrator-free in some deployments. If the
    # orchestrator call failed (network error, no service in cluster, etc.)
    # fall back to the in-process local evaluator so the UI still gets real
    # numerator/denominator counts instead of "Not evaluated".
    if use_native_cql_engine and native_report is None:
        try:
            from local_measures import evaluate_all_measures  # type: ignore

            bundle_for_local: Dict[str, Any]
            if isinstance(patient_blob, dict) and patient_blob.get("resourceType") == "Bundle":
                bundle_for_local = patient_blob
            else:
                # Reconstruct a minimal Bundle from the extracted FHIR view so
                # local_measures._bundle_resources can group resources by type.
                entries: List[Dict[str, Any]] = []
                if isinstance(fhir_view, dict):
                    patient_res = fhir_view.get("patient")
                    if isinstance(patient_res, dict):
                        entries.append({"resource": patient_res})
                    for key in ("encounters", "conditions", "observations", "procedures", "coverages"):
                        items = fhir_view.get(key) or []
                        if isinstance(items, list):
                            for item in items:
                                if isinstance(item, dict):
                                    entries.append({"resource": item})
                bundle_for_local = {"resourceType": "Bundle", "entry": entries}

            local_report = evaluate_all_measures(
                bundle_for_local,
                period_start=base_payload.get("measurement_period_start", "2025-01-01"),
                period_end=base_payload.get("measurement_period_end", "2025-12-31"),
            )
            wanted = {str(m).lower() for m in measure_ids} if measure_ids else None
            filtered = [
                m for m in (local_report.get("measures") or [])
                if not wanted or str(m.get("measureId", "")).lower() in wanted
            ]
            in_denominator = sum(1 for m in filtered if m.get("denominator"))
            in_numerator = sum(1 for m in filtered if m.get("numerator"))
            gaps = [
            ...
            native_report = {
                ...
                "summary": {
                    ...
                    "controlled": in_numerator,
                    "gaps_in_care": gaps,
                },
            }
        except Exception as e:  # noqa: BLE001
            remote_errors.append({
                "url": "in-process:local_measures",
    ...
    native_is_local = (
        isinstance(native_report, dict) and native_report.get("engine") == "local-stub"
    )
    ...
    return {
        ...
        "mode": "engine-toggles",
        "executionSource": (
            "local-stub" if native_is_local and not ai_report
            else "digital-quality-orchestrator" if (native_report or ai_report)
            else "none"
        ),
        "engines": {
...
class FileBackedCosmosDBHelper:
    ...
    def save_patient_data(self, patient_id: str, patient_data: Dict[str, Any]) -> bool:
        ...
        # Keep the docType=patient view in sync so list_docs("patient") /
        # list_docs("member") (used by the workbench /members endpoint) can
        # see anything written via save_patient_data, not just _store.
        self.upsert_doc("patient", patient_id, document)
        return True
...
def _resolve_sample_data_dir() -> Path:
    ...
    candidates = [
        base.parent / "_data",
        base.parent.parent / "_data",
        base.parent.parent.parent / "_data",
        base.parent / "data",
        base.parent.parent / "data",
    ]
    # Require the workbench seed marker so a stray per-app `_data/` holding
    # only e.g. `consumers_soap_notes.json` doesn't shadow the platform-root
    # `_data/` that actually has cohorts.json, patients.json, etc.
    for candidate in candidates:
        if candidate.exists() and (candidate / "cohorts.json").exists():
            return candidate
    for candidate in candidates:
        if candidate.exists():
            return candidate
...
# ---------------------------------------------------------------------------
# Patients tab: SOAP notes + sample-patient bundles (shared with consumers)
# ---------------------------------------------------------------------------
try:
    from soap_notes import create_soap_router
    ...
    print("✓ SOAP notes router registered at /api/patients/{id}/soap-notes")
except Exception as e:  # noqa: BLE001
    print(f"⚠ Could not register SOAP notes router: {e}")

try:
    from sample_patients import create_sample_patients_router

    app.include_router(
        create_sample_patients_router(
            sample_data_dir=sample_data_dir,
            auth_dependency=get_current_user_conditional,
        )
    )
    print("✓ Sample-patients router registered at /api/sample-patients")
except Exception as e:  # noqa: BLE001
    print(f"⚠ Could not register sample-patients router: {e}")
