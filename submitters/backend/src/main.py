...
def _evaluate_bp_measurement_via_orchestrator(
    ...
    *,
    ...
    # Submitters stack runs orchestrator-free in the current AKS deployment.
    # When the orchestrator POST fails (DNS / connection / no service),
    # fall back to the in-process local evaluator so per-member rows render
    # real numerator/denominator counts instead of "Not evaluated".
...
try:
    ...
    try:
        from measurement_history import record_measurement_history as _record_measurement_history
    except Exception as _e:  # noqa: BLE001
        print(f"⚠ measurement_history module unavailable: {_e}")
        _record_measurement_history = None

    def _deqm_record_history(**fields: Any) -> None:
        if _record_measurement_history is None:
            return
        try:
            _record_measurement_history(cohortsDBHelper, **fields)
        except Exception as e:  # noqa: BLE001
            print(f"⚠ Failed to record measurement_history (direct): {e}")
    ...
    app.include_router(
        create_deqm_router(
            ...
            record_history=_deqm_record_history,
        )
    )
